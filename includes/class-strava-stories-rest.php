...
class Strava_Stories_Rest {
	...
	private static function format_distance( float $meters, bool $metric, bool $long = false ): string {
		$unit = $metric
			? ( $long ? 'kilometers' : 'km' )
			: ( $long ? 'miles' : 'mi' );
		return sprintf( '%.2f %s', $metric ? $meters / 1000 : $meters / 1609.344, $unit );
	}
	...
	private static function draft_stats( array $a ): array {
		...
		$out = array();
		if ( $distance_m > 0 ) {
			$out[] = self::format_distance( $distance_m, $is_metric, true );
		}
		if ( $moving_s > 0 ) {
